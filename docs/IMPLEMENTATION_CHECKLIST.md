...
- [x] 홈 페이지 (`/`) 구현
- [x] 최신 글 6개 카드 목록 (블로그 + 위키 통합, 날짜 내림차순)
- [x] 글 카드 컴포넌트 (타입 배지, 제목, 설명, 날짜, 카테고리)
- [x] 타입 배지(`Blog` / `Wiki`) 색상 구분 표시
- [x] "Blog 전체 보기" / "Wiki 전체 보기" 버튼 (각각 `/blog/`, `/wiki/`)
...
- [x] 홈 페이지에 위키도 통합 표시 (Blog/Wiki 타입 배지로 구분)
