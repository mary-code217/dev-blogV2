---
title: "정상이 뭔지 모르면 장애도 못 본다 — 모니터링 환경 만들기"
description: "장애 대응 경험을 만들기 위해 메트릭·로그 2축 관측 환경을 직접 구성하고, 이후 모든 장애 판단의 기준이 될 baseline을 확보한 기록. 장애 실습 시리즈 1편."
date: 2026-07-08
category: "Backend"
...
>
> 이 글은 **"장애를 직접 내보며 배우는 관측성" 4부작의 1편**이다.
...
모니터링 글은 많지만 대부분 "Grafana 설치하고 대시보드 띄우기"에서 끝난다. 이 시리즈는 그 다음, 즉 **대시보드가 실제로 비명을 지르는 순간부터**를 다룬다. 다만 그 전에 반드시 해야 하는 일이 있었다. 이 글의 주제 — **정상 상태가 어떤 모양인지부터 아는 것**이다.

## 도구는 질문에서 골랐다

장애를 일으키고 나면 답해야 할 질문은 결국 두 개였다.

- **"언제부터 이상했는가?"** — 추세를 보여주는 축이 필요하다 → **메트릭**
- **"왜 이상했는가?"** — 개별 요청의 맥락을 담는 축이 필요하다 → **로그**
...
- **트레이스(Tempo 등)는 뺐다.** 단일 서비스 + DB 하나 구조라 메트릭·로그 2축으로 탐지→원인 추적이 닫힌다고 판단했다. 이 판단이 맞았는지는 4편에서 돌아본다.
...
**4. scrape 주기는 5초로 줄였다.** 기본 15초로는 빠르게 진행되는 장애의 결정적 구간을 놓칠 수 있다고 봤는데, 이 선택은 2편에서 바로 보상받는다 — 커넥션 풀 대기열이 **첫 scrape 5초 만에 0→45로 점프**하는 걸 잡아냈다.
...
로그 축도 끝까지 검증했다. 요청 하나를 보내면 응답 헤더로 `X-Request-Id`가 돌아오고, 그 값을 Grafana 대시보드의 requestId 변수에 넣으면 해당 요청의 요청·응답 로그 2라인이 정확히 걸린다.
...
→ [2편. 자원이 마르면 생기는 일 — 커넥션 풀 고갈과 느린 쿼리](/dev-blogV2/blog/observability-lab-02-resource/)
