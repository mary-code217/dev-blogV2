...
TPS와 함께 자주 등장하는 용어도 정리해두면 좋습니다.
...
WAS를 아무리 늘려도 결국 DB가 1대면 거기서 병목이 생깁니다.
...
읽기(조회)는 캐싱으로 해결할 수 있지만, 쓰기(주문, 결제, 재고 차감)는 무조건 DB를 거쳐야 합니다. 트래픽이 급증하면 캐시 히트율이 높아도 미스 절대량이 많아져서 DB가 버티지 못합니다.
...
가장 먼저 해볼 수 있는 방법입니다. DB 호출 횟수를 줄이는 것만으로 큰 효과를 볼 수 있습니다.
...
핵심 로직만 동기로 처리하고, 나머지는 이벤트로 넘기면 응답 시간이 확 줄어듭니다.
...
충돌이 자주 발생하지 않는 상황이라면 낙관적 락으로 전환하는 것만으로 동시 처리량이 올라갑니다.
...
대부분의 서비스는 읽기가 80~90%를 차지합니다. 읽기를 분리하는 것만으로도 DB 부하가 크게 줄어듭니다.
...
**서버를 사기 전에 코드부터 봐야 합니다.** 비용 0원짜리 최적화가 생각보다 효과가 큽니다.
...
※ HikariCP 문서(brettwooldridge)에서 소개된 공식. PostgreSQL 환경 기준이며 어디까지나 출발점입니다. 실제 값은 부하 테스트로 조정이 필요합니다.
```
...
- **비용 0원인 최적화가 효과 큽니다** - 벌크 처리, 비동기, 락 최적화
- **커넥션 풀 부족은 증상입니다** - 진짜 원인은 느린 쿼리
- **샤딩은 최후의 수단** - 복잡도가 급격히 증가하고, 대기업이나 하는 이유가 있습니다
- **"샤딩 해야 할까요?"라고 물어보는 단계면 안 해도 됩니다**
