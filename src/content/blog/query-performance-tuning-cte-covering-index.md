...
테이블 17개와 CTE 16개를 사용하는 쿼리 하나가 수백 줄이었습니다. 처음 실행했을 때 **30초 이상** 걸려 실사용은 불가능한 수준이었습니다.
...
SELECT 절에 스칼라 서브쿼리가 잔뜩 달려 있었습니다. 문제는 **행마다 실행된다**는 점입니다.

- 페이징으로 20행을 가져오면
- 서브쿼리 15개 × 20행 = **300번 테이블 접근**
- 그중 한 영역은 같은 테이블을 15번이나 스캔하고 있었습니다

Java의 N+1 문제와 같은 원리입니다. 메인 쿼리의 각 행마다 서브쿼리가 따로따로 실행되니까, 행이 늘어나면 서브쿼리 실행 횟수도 그만큼 늘어납니다. 20행이면 300회, 40행이면 600회입니다.
...
SELECT에서 행마다 서브쿼리를 실행하는 대신, WITH절에서 영역별로 **미리 집계**해놓고 메인에서 LEFT JOIN합니다.
...
**기준 CTE에서 페이징을 먼저 적용**합니다. `WITH_BASE`에서 20건만 걸러놓으면, 이후 CTE들은 `WHERE key IN (SELECT key FROM WITH_BASE)` 조건으로 20건에 해당하는 데이터만 가져옵니다. 전체 테이블을 스캔할 필요가 없어집니다.
...
30초 이상 걸리던 게 10초 이내로 줄었습니다. 인덱스는 아직 손대지 않은 상태입니다.

---

## 2차. ROW_NUMBER와 PIVOT으로 테이블 스캔을 1회로

CTE로 바꿨는데도 특정 영역이 여전히 느렸습니다. 원인을 파보니 CTE 안에서 스칼라 서브쿼리를 쓰고 있었습니다.
...
각 종류마다 ID, 연도, 지역코드1, 지역코드2, 결재여부를 가져와야 해서, 서브쿼리가 **15개**(5개 × 3종류)였습니다.
...
같은 `PARTITION BY`에 정렬 방향만 다른 ROW_NUMBER를 동시에 계산합니다. `rn_asc = 1`인 행은 가장 오래된 데이터이고, `rn_desc = 1`인 행은 가장 최근 데이터입니다.
...
`MAX(CASE WHEN 조건 THEN 값 END)` 패턴으로, 한 번의 GROUP BY에서 여러 종류의 값을 한 행으로 PIVOT합니다. 이렇게 하면 key 하나당 정확히 1행이 나오고, 메인 쿼리에서 key 하나로 LEFT JOIN하면 끝납니다.
...
## 3차. 커버링 인덱스로 테이블 액세스 제거

쿼리 구조를 바꾼 뒤, 인덱스도 손봤습니다.

### 인덱스만 읽어 테이블 액세스를 줄인다
...
두 번의 쿼리 구조 변경으로 조회 시간을 30초 이상에서 6\~8초로 줄였습니다. 이후 인덱스를 적용해 5\~6초로 줄였습니다. 쿼리가 느릴 때 인덱스부터 뒤지는 습관이 있었는데, 이 쿼리에서는 순서가 반대였습니다.
