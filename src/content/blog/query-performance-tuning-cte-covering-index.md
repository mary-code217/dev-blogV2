...
여러 항목의 현황을 한 화면에 보여주는 대시보드를 만들고 있었습니다. 하나의 목록 행에 9개 영역의 상태값이 들어가야 해서, 통합 조회 쿼리 하나로 전부 가져오는 구조였습니다.

```
[목록 1행] = 기본정보 + 영역A 상태 + 영역B 상태 + ... + 영역I 상태
```

테이블은 17개, CTE는 16개. 쿼리 하나가 수백 줄이었습니다. 처음 실행했을 때 **30초 이상**. 실사용은 불가능한 수준이었습니다.
...
SELECT 절에 스칼라 서브쿼리가 잔뜩 달려 있었습니다. 문제는 **스칼라 서브쿼리는 행마다 실행된다**는 점입니다.

- 페이징으로 20행을 가져오면
- 서브쿼리 15개 × 20행 = **300번 테이블 접근**
- 그중 한 영역은 같은 테이블을 15번이나 스캔하고 있었습니다

Java의 N+1 문제와 같은 원리입니다. 메인 쿼리의 각 행마다 서브쿼리가 따로따로 실행되니까, 행이 늘어날수록 기하급수적으로 느려집니다.

---

## 1차 개선: 서브쿼리 → CTE 리팩토링

동료에게 느린 쿼리를 공유했더니, **CTE(WITH절)로 바꿔보라**는 조언을 받았습니다.

핵심 아이디어는 단순합니다. SELECT에서 행마다 서브쿼리를 실행하는 대신, WITH절에서 영역별로 **미리 집계**해놓고 메인에서 LEFT JOIN하는 것입니다.
...
여기서 중요한 포인트가 하나 있습니다. **기준 CTE에서 페이징을 먼저 적용**한다는 것입니다. `WITH_BASE`에서 20건만 걸러놓으면, 이후 CTE들은 `WHERE key IN (SELECT key FROM WITH_BASE)` 조건으로 20건에 해당하는 데이터만 가져옵니다. 전체 테이블을 스캔할 필요가 없어집니다.
...
**결과: 30초 이상 → 10초 이내.** 구조 변경만으로 큰 폭의 개선이 있었습니다.

---

## 2차 개선: ROW_NUMBER + PIVOT

CTE로 바꿨는데도, 특정 영역이 여전히 느렸습니다. 원인을 파보니 CTE **안에서** 여전히 스칼라 서브쿼리를 쓰고 있었습니다.

이 영역은 하나의 테이블에서 3가지 종류의 데이터를 뽑아야 했습니다.

- 종류1: `type = 'R'`인 것 중 **가장 오래된** 것
- 종류2: `type = 'R'`인 것 중 **가장 최근** 것
- 종류3: `type = 'F'`인 것 중 **가장 최근** 것

각 종류마다 ID, 연도, 지역코드1, 지역코드2, 결재여부를 가져와야 해서, 서브쿼리가 **15개**(5개 × 3종류)였습니다. 같은 테이블을 15번 스캔하고 있었던 것입니다.
...
**1단계**: 테이블을 딱 1번만 스캔하면서 ROW_NUMBER를 부여합니다.
...
같은 `PARTITION BY`에 정렬 방향만 다른 ROW_NUMBER를 동시에 계산합니다. `rn_asc = 1`이면 가장 오래된 것, `rn_desc = 1`이면 가장 최근 것입니다.

**2단계**: `rn = 1`인 행만 필터한 뒤, MAX(CASE WHEN)으로 PIVOT 집계합니다.
...
`MAX(CASE WHEN 조건 THEN 값 END)` 패턴으로, 한 번의 GROUP BY에서 여러 종류의 값을 한 행으로 PIVOT합니다. 이렇게 하면 key 하나당 정확히 1행이 나오고, 메인 쿼리에서 깔끔하게 LEFT JOIN할 수 있습니다.
...
쿼리 구조를 바꾼 뒤, 인덱스도 손봤습니다.

### 커버링 인덱스란?

일반적인 인덱스는 **WHERE 조건에 맞는 행을 빨리 찾는 것**이 목적입니다. 인덱스로 위치를 찾은 뒤, 실제 데이터는 테이블에서 다시 읽어옵니다.
...
커버링 인덱스는 쿼리가 필요로 하는 **모든 컬럼**을 인덱스에 포함시킵니다. 그러면 인덱스만 읽고 테이블은 아예 안 가도 됩니다. 디스크 I/O가 크게 줄어듭니다.

### 적용한 인덱스들

총 7개 인덱스를 생성했습니다. 이 중 2개는 커버링 인덱스, 나머지 5개는 조인/필터 성능 개선용입니다.
...
이 순서를 지켜야 인덱스 스캔 범위가 최소화됩니다. 선두 컬럼이 WHERE 조건과 일치해야 인덱스 범위 스캔(Index Range Scan)이 가능하기 때문입니다.
...
이 인덱스들은 커버링까지는 아니지만, JOIN이나 WHERE 조건에서 Full Table Scan을 방지합니다.

다만 커버링 인덱스는 컬럼이 많아지는 만큼 INSERT/UPDATE/DELETE 시 인덱스 유지 비용이 늘어납니다. 이 화면은 조회 전용 대시보드라 DML이 거의 없어서 적합했지만, 쓰기가 빈번한 테이블이라면 신중하게 판단해야 합니다.
...
- **스칼라 서브쿼리는 행마다 실행됩니다.** Java의 N+1과 같은 원리입니다. 쿼리 안에 서브쿼리가 있으면 가장 먼저 의심해볼 것.
- **CTE로 선집계 후 JOIN하면 테이블 접근 횟수가 급감합니다.** 300회 → 11회. 구조만 바꿔도 이 정도 차이가 납니다.
- **ROW_NUMBER + PIVOT 패턴은 같은 테이블에서 여러 종류의 값을 뽑을 때 강력합니다.** 1회 스캔으로 모든 종류를 동시에 처리합니다.
- **커버링 인덱스는 "테이블을 아예 안 읽게" 만듭니다.** WHERE/ORDER BY뿐 아니라 SELECT 절의 컬럼까지 인덱스에 넣으면 Index Only Scan이 가능해집니다.
- **페이징은 가장 먼저 적용해야 합니다.** 기준 CTE에서 20건만 걸러놓으면, 이후 CTE들은 20건만 처리하면 됩니다.
