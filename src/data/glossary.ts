...
export const glossary: GlossaryTerm[] = [
	...
	},
	{
		...
		category: 'AI',
		description:
			'Parameter. 모델이 학습하면서 스스로 조정하는 숫자. 입력에 곱하는 가중치(weight)와 계산 결과에 더하는 편향(bias)이 대표적이다. 가중치는 음수도 될 수 있어서 입력을 얼마나, 어느 방향으로 반영할지를 함께 정한다. LLM 이름에 붙는 "7B"는 이 숫자가 70억 개라는 뜻이다.',
	},
	{
		term: '손실 함수',
		category: 'AI',
		description:
			'Loss Function. 예측이 정답과 얼마나 다른지를 점수 하나로 만드는 함수. 학습은 이 점수(손실)가 줄어드는 방향으로 파라미터를 바꾸는 과정이다. 오차를 제곱해 평균 낸 MSE(평균 제곱 오차)는 회귀 문제에, 교차 엔트로피는 LLM 같은 분류·생성 문제에 주로 쓴다.',
	},
	{
		term: '교차 엔트로피',
		category: 'AI',
		description:
			'Cross-Entropy. 모델이 예측한 확률 분포가 정답과 얼마나 어긋나는지 재는 손실. 다음 토큰 예측에서는 실제 정답 토큰에 준 확률이 낮을수록 값이 커진다. 정답에 90%를 줬으면 손실이 작고, 1%를 줬으면 크게 튄다.',
	},
	{
		term: '기울기',
		category: 'AI',
		description:
			'Gradient. 파라미터 하나를 아주 조금 바꿨을 때 손실이 어느 방향으로, 얼마나 빠르게 변하는지를 파라미터마다 구해 모아 둔 값. 어떤 파라미터의 기울기가 양수면 그 값을 줄여야 손실이 줄어든다. 역전파로 계산하고 옵티마이저가 업데이트에 쓴다.',
	},
	{
		term: '학습률',
		category: 'AI',
		description:
			'Learning Rate. 기울기를 보고 파라미터를 한 번에 얼마나 크게 바꿀지 정하는 계수. 너무 작으면 학습이 느리고, 너무 크면 손실이 가장 작은 지점을 지나쳐 손실이 오히려 커지며 발산할 수 있다. 대표적인 하이퍼파라미터다.',
	},
	{
		term: '하이퍼파라미터',
		category: 'AI',
		description:
			'Hyperparameter. 학습으로 정해지는 파라미터와 달리 사람이 학습 전에 정하는 설정값. 학습률, 배치 크기, 층 수처럼 학습 방식이나 모델 구조를 결정한다. 보통 검증 데이터에서 성능을 비교해 고른다.',
	},
	{
		term: '추론(Inference)',
		category: 'AI',
		description:
			'학습을 마친 모델에 새 입력을 넣어 결과만 계산하는 단계로, 가중치는 바뀌지 않는다. LLM과 대화하는 것도 추론이라 대화 중 지시를 따르는 건 입력 문맥이 달라져서지 모델이 새로 학습해서가 아니다. 모델이 단계를 밟아 생각하는 reasoning도 한국어로 추론이라 부르지만 다른 개념이다.',
	},
	{
		term: '활성화 함수',
		category: 'AI',
		description:
			'Activation Function. 뉴런의 계산 결과에 비선형 변환을 거는 함수. 곱하고 더하는 계산(선형 변환)만 여러 층 쌓으면 결국 선형 변환 하나로 합쳐지기 때문에, 층을 깊게 쌓아 복잡한 관계를 표현하려면 이 비선형성이 필요하다. 음수는 0으로, 양수는 그대로 내보내는 ReLU가 대표적이다.',
	},
	{
		term: '순전파·역전파',
		category: 'AI',
		description:
			'Forward / Backward Propagation. 순전파는 입력에서 출력 쪽으로 계산을 진행하는 과정이고, 학습 때는 그 예측으로 손실까지 구한다. 역전파는 반대로 손실에서 입력 쪽으로 거슬러 올라가며 미분의 연쇄법칙으로 각 파라미터의 기울기를 구한다. 역전파는 기울기 계산까지만 하고, 값을 실제로 바꾸는 건 옵티마이저다.',
	},
	{
		term: '옵티마이저',
		category: 'AI',
		description:
			'Optimizer. 역전파로 구한 기울기를 써서 파라미터를 실제로 업데이트하는 규칙. 가장 기본은 기울기 반대 방향으로 학습률만큼 옮기는 경사하강법(Gradient Descent)이다. 데이터를 조금씩 무작위로 뽑아 기울기를 구하고 업데이트하는 SGD(확률적 경사하강법), 파라미터마다 이동 폭을 자동으로 조절하는 Adam이 널리 쓰인다.',
	},
	{
		term: '과적합',
		category: 'AI',
		description:
			'Overfitting. 모델이 학습 데이터에 지나치게 맞춰져 처음 보는 데이터에서는 성능이 떨어지는 현상. 기출문제 답만 외운 상태와 비슷하다. 학습 손실은 계속 줄어드는데 검증 손실이 다시 오르기 시작하면 과적합을 의심한다.',
	},
	{
		term: '검증·테스트 데이터',
		category: 'AI',
		description:
			'Validation / Test Set. 학습에 쓰지 않고 따로 떼어 둔 평가용 데이터. 검증 데이터는 학습률이나 모델 구성을 고르는 데 쓰고, 테스트 데이터는 선택을 모두 마친 뒤 최종 성능을 재는 데 남겨 둔다. 테스트 점수를 보며 모델을 계속 고치면 테스트 데이터도 사실상 검증 데이터가 된다.',
	},
	{
		term: '데이터 누수',
		category: 'AI',
		description:
			'Data Leakage. 실제 사용 시점에는 알 수 없는 정보가 학습이나 평가에 섞여 성능이 부풀려지는 문제. 같은 문서를 조금 바꾼 사본이 학습과 테스트 양쪽에 들어가거나, 미래 매출을 예측하면서 미래 데이터를 학습에 쓰는 경우가 대표적이다.',
	},
	{
		term: '토큰',
		category: 'AI',
		description:
			'Token. LLM이 텍스트를 처리하는 단위. 단어 전체일 수도, 단어 일부나 문장부호일 수도 있다. 한국어 한 글자가 항상 한 토큰은 아니라서 글자 수와 토큰 수는 다르다. API 요금과 컨텍스트 윈도우 한도가 모두 토큰 수 기준이다.',
	},
	{
		term: '토크나이저',
		category: 'AI',
		description:
			'Tokenizer. 텍스트를 토큰으로 쪼개고 각 토큰에 ID를 붙이는 도구. ID는 어휘 목록(vocabulary)의 항목 번호일 뿐이라 숫자가 가깝다고 의미가 가깝지 않다. 같은 문장도 모델마다 토크나이저가 달라 토큰 수가 다르게 나온다.',
	},
	{
		term: '자기회귀 생성',
		category: 'AI',
		description:
			'Autoregressive Generation. 다음 토큰 하나를 예측해 고르고, 그 토큰을 문맥 뒤에 붙인 뒤 다시 다음 토큰을 예측하는 반복으로 문장을 만드는 방식. LLM 응답이 조금씩 흘러나오는 스트리밍이 이 구조에서 나온다.',
	},
	{
		term: '자기지도학습',
		category: 'AI',
		description:
			'Self-Supervised Learning. 사람이 정답을 따로 달지 않고 데이터 자체에서 학습 문제를 만들어 배우는 방식. LLM 사전학습에서는 문장의 다음 토큰이 곧 정답이라 대량의 텍스트를 그대로 학습 재료로 쓸 수 있다. 이미지 등 다른 데이터에도 쓰인다.',
	},
	{
		term: '트랜스포머',
		category: 'AI',
		description:
			'Transformer. 현재 LLM 대부분이 쓰는 신경망 구조. 토큰 사이 정보를 섞는 어텐션과, 토큰 위치마다 표현을 변환하는 피드포워드 신경망(FFN)을 한 블록으로 묶고, 잔차 연결과 레이어 정규화를 더해 이 블록을 여러 층 쌓는다. 순서 정보는 위치 인코딩으로 넣는다. 2017년 논문 "Attention Is All You Need"에서 처음 제안됐다.',
	},
	{
		term: '어텐션',
		category: 'AI',
		description:
			'Attention. 각 토큰이 문맥 속 토큰들(자기 자신 포함)의 정보를 얼마나 가져올지 계산해 섞는 연산. 토큰마다 Q(Query, 찾는 조건), K(Key, 검색 표식), V(Value, 가져올 정보) 벡터를 만들고, Q와 K를 비교한 점수를 비중 삼아 V를 가중합(비중만큼 곱해 더하는 것)한다. 이 계산을 여러 벌 병렬로 돌려 합치는 것이 멀티헤드 어텐션이다.',
	},
	{
		term: '인과적 마스킹',
		category: 'AI',
		description:
			'Causal Masking. 어텐션에서 각 토큰이 자기보다 뒤에 오는 토큰을 보지 못하게 가리는 장치. 다음 토큰을 예측하는 모델이 학습 중에 정답(미래 토큰)을 미리 보는 것을 막는다. GPT 계열 같은 디코더형 생성 모델이 이 방식을 쓴다.',
	},
	{
		term: '잔차 연결',
		category: 'AI',
		description:
			'Residual Connection. 층의 입력을 층의 출력에 그대로 더해 주는 우회 경로. 층은 원래 값을 통째로 다시 만들 필요 없이 바뀔 부분만 학습하면 되고, 층이 깊어져도 역전파 기울기가 앞쪽까지 잘 전달된다. 스킵 연결(skip connection)이라고도 한다.',
	},
	{
		term: '레이어 정규화',
		category: 'AI',
		description:
			'Layer Normalization. 층을 지나며 커지거나 작아진 값의 크기를 일정한 범위로 맞추는 기법. 토큰 하나의 벡터 안에서 평균이 0, 표준편차(값이 퍼진 정도)가 1이 되게 맞춘 뒤 학습한 배율로 다시 조정한다. 학습을 안정시키는 용도이며 DB 정규화와는 무관하다.',
	},
	{
		term: '위치 인코딩',
		category: 'AI',
		description:
			'Positional Encoding. 토큰의 순서 정보를 모델에 넣는 방법. 어텐션은 순서 정보 없이는 "개가 사람을 물었다"와 "사람이 개를 물었다"를 제대로 구분하지 못한다. 처음 제안된 트랜스포머는 위치별 벡터를 임베딩에 더했고, 최근 모델은 Q·K 벡터를 위치에 따라 회전시키는 RoPE(Rotary Position Embedding)를 많이 쓴다.',
	},
	{
		term: '로짓·소프트맥스',
		category: 'AI',
		description:
			'Logit / Softmax. 로짓은 모델이 다음 토큰 후보마다 매긴 원점수로, 음수도 되고 합이 1도 아니다. 소프트맥스는 이 점수들을 0~1 사이이면서 합이 1인 확률로 바꾸는 함수다. 점수 차이가 클수록 1등 후보에 확률이 더 몰린다.',
	},
	{
		term: '온도',
		category: 'AI',
		description:
			'Temperature. 소프트맥스에 넣기 전에 로짓을 나누는 양수 값. 낮추면 높은 점수 후보에 확률이 몰려 답이 일정해지고, 높이면 분포가 고르게 퍼져 답이 다양해진다. 고르는 방식을 바꿀 뿐이라 높인다고 모델이 똑똑해지지 않고, 낮춘다고 틀린 답이 맞는 답이 되지도 않는다.',
	},
	{
		term: '탐욕적 디코딩',
		category: 'AI',
		description:
			'Greedy Decoding. 매 단계 확률이 가장 높은 토큰 하나만 고르는 생성 방식. 이론상 같은 입력에 늘 같은 답이 나오지만 같은 구절을 되풀이하는 문제가 생기기 쉽다. 확률에 따라 무작위로 뽑는 샘플링(sampling)과 대비되며, 온도를 0에 가깝게 낮추면 이 방식에 가까워진다.',
	},
	{
		term: '컨텍스트 윈도우',
		category: 'AI',
		description:
			'Context Window. 모델이 한 번에 참고할 수 있는 최대 토큰 수. 시스템 지시, 이전 대화, 검색해 넣은 문서, 지금 생성 중인 답변이 모두 이 한도를 나눠 쓴다. 한도 안에 들어가더라도 긴 문맥의 정보를 고르게 잘 활용한다는 보장은 없다.',
	},
	{
		term: '사전학습',
		category: 'AI',
		description:
			'Pretraining. 대규모 텍스트로 다음 토큰 예측을 반복하며 언어 패턴과 폭넓은 지식을 익히는 첫 학습 단계. 비용이 가장 많이 드는 단계이고, 결과물인 기본 모델(base model)은 문장을 이어 쓸 줄은 알아도 지시를 따르는 데는 서툴다.',
	},
	{
		term: '파인튜닝',
		category: 'AI',
		description:
			'Fine-tuning. 사전학습된 모델을 특정 작업이나 도메인 데이터로 추가 학습해 파라미터를 조정하는 것. 문의 분류나 정해진 출력 형식처럼 동작 방식을 맞추는 데 적합하고, 자주 바뀌는 정보를 정확히 꺼내 쓰는 용도로는 RAG가 더 맞다. 문서를 프롬프트에 넣는 것은 파라미터가 바뀌지 않으므로 파인튜닝이 아니다.',
	},
	{
		term: 'LoRA',
		category: 'AI',
		description:
			'Low-Rank Adaptation. 기존 가중치는 고정해 두고, 가중치 변화량을 작은 행렬 두 개의 곱으로 표현해 그 두 행렬만 학습하는 파인튜닝 기법. 두 행렬 사이의 차원인 랭크(r)가 작을수록 학습할 숫자가 줄어든다. 숫자 100만 개가 든 1,000×1,000 행렬의 변화량을 랭크 8이면 1만 6천 개 숫자로 학습한다. 학습 후에는 기존 가중치에 합쳐서 추론할 수 있다.',
	},
	{
		term: '환각',
		category: 'AI',
		description:
			'Hallucination. 모델이 사실과 다르거나 근거 없는 내용을 사실처럼 그럴듯하게 생성하는 현상. 다음 토큰 예측은 자연스러운 문장을 고르는 과정이지 사실을 검증하는 과정이 아니라서 생긴다. RAG로 근거를 넣어도 근거를 잘못 읽거나 없는 내용을 덧붙일 수 있다.',
	},
];
